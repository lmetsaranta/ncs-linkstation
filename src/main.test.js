...
test("Correct printing when linkstation is reachable", () => {
  ...
  expect(main(linkstations, device)).toBe("Best link station for point 0,0 is 0,0 with power 100.00");
});
...
test("Best linkstation is chosen when multiple stations within reach", () => {
  ...
  expect(main(linkstations, device)).toBe("Best link station for point 0,0 is 0,1 with power 9.00");
});
...
test("Negative coordinate values are calculated correctly", () => {
  ...
  const linkstations = {};
  linkstations.stationArray = [linkstation];
  expect(main(linkstations, device)).toBe("Best link station for point -2,-2 is -2,0 with power 16.00");
});
